...
class DeBlock(nn.Module):
    """Enhanced decoder block with LAP-style processing"""
    def __init__(self, in_channels, out_channels):
...
class LaplacianFINet(nn.Module):
    """FINet with Laplacian Pyramid Integration"""
    ...
    def __init__(self, backbone='efficientb0', channels=(8, 12, 24, 48)):
        ...
        # Laplacian injection blocks for each encoder stage
        ...
        # Enhanced frequency fusion modules
        ...
        # Enhanced decoder with LAP-style processing
    ...
    def forward(self, x, high, low):
        # Generate Laplacian pyramid from input
        laplacian_levels = self.laplacian_pyramid(x)
        
        # Forward through encoder with Laplacian injection
        x0, x1, x2, x3, x4 = self.encoder(x)
        
        # Inject Laplacian levels at corresponding scales
        # Note: We match pyramid levels to encoder stages based on spatial resolution
        ...
        # Hierarchical decoding with LAP processing
        out3 = self.gelu(
...
if __name__ == '__main__':
    # Test the model
    model = LaplacianFINet(backbone='efficientb0', channels=(8, 24, 32, 64))
    
    # Create dummy inputs
    x = torch.randn(1, 3, 384, 384)
    high = torch.randn(1, 96, 48, 48)
    low = torch.randn(1, 96, 48, 48)
