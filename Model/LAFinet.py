...
LDSConv wiring (new)
------------------------------------------------------------------
StarLapInjection (all three injection sites) and the two
*highest-resolution* StarDeBlock decoder stages (deconv2, deconv1) now
use LDSConv internally instead of a static 7×7 depthwise conv — see
Model/star_utils.py for the rationale.

deconv3 (the lowest-resolution, highest-channel decoder stage, operating
on channels[3] -> channels[2]) deliberately keeps the original static
depthwise conv: it's about coarse localization rather than fine boundary
shape, so LDSConv's extra cost (offset prediction + LKP + bilinear
gather) isn't justified there. `decoder_ldsconv_stages` controls this
explicitly so it's easy to ablate.
"""
...
class LaplacianFINet(nn.Module):
    ...
    decoder_ldsconv_stages: which decoder StarDeBlocks use LDSConv on
        their low branch instead of a static depthwise conv. Default
        skips deconv3 (lowest-res / highest-channel stage) and applies
        it to deconv2 + deconv1, where boundary precision matters most
        and the extra compute is cheapest. Pass e.g. () to disable
        everywhere, or ('deconv3','deconv2','deconv1') to apply
        everywhere (not recommended — see star_utils.py docstring).
        Only takes effect when use_star=True.
    """
    ...
    def __init__(
        ...
        decoder_ldsconv_stages: tuple = ('deconv2', 'deconv1'),
    ):
        super().__init__()
        ...
        if use_star:
            # LDSConv enabled by default in StarLapInjection (conditioned on
            # the Laplacian projection) — this is where deformable, edge-
            # aware sampling matters most, so all three sites use it.
        ...
        if use_star:
            self.deconv3 = StarDeBlock(channels[3], channels[2],
                                        use_ldsconv=('deconv3' in decoder_ldsconv_stages))
            self.deconv2 = StarDeBlock(channels[2], channels[1],
                                        use_ldsconv=('deconv2' in decoder_ldsconv_stages))
            self.deconv1 = StarDeBlock(channels[1], channels[0],
                                        use_ldsconv=('deconv1' in decoder_ldsconv_stages))
        else:
