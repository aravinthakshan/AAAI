...
class DemoNetEncoder(nn.Module):
    def __init__(self, depth=12, dim=64, drop_path_rate=0.,
                 layer_scale_init_value=1e-6, mode="mul", **kwargs):
        super().__init__()
        assert mode in ["sum", "mul"]
        self.dim = dim
        ...
        for split_size in split_sizes:
            blocks.append(nn.Sequential(*[
                Block(dim=dim, drop_path=dp_rates[start + i],
                      layer_scale_init_value=layer_scale_init_value, mode=mode)
    ...
    def forward(self, x):
        ...
        x = features[0]
        for downsample, blocks in zip(self.downsamples, self.blocks):
            x = downsample(x).permute(0, 2, 3, 1)
            x = blocks(x).permute(0, 3, 1, 2)
            features.append(x)
